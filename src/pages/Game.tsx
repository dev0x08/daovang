        ...
        <footer className="game-bottom-hud redesigned-bottom-hud">
            <div className="player-hand-row">
                <section className="player-progress-card">
                    <div className="progress-avatar-wrap"><button type="button" className={`progress-avatar ${cosmeticClass(profile?.equipped.frame)}`} aria-label="Mở menu người chơi" onClick={() => setMobilePlayerMenuOpen(v => !v)}>{profile?.photoURL ? <img src={profile.photoURL} alt="" /> : <UserRound />}</button>{profile?.equipped.badge && <i className={`identity-badge ${cosmeticClass(profile.equipped.badge)}`}><Shield /></i>}<i className="online-dot" /></div>
