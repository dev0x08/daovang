...
const playGameCue=async(kind:'start'|'turn')=>{
    try{
        const context=gameAudioContext||(gameAudioContext=new AudioContext());
        if(context.state==='suspended')await context.resume();
        const now=context.currentTime,master=context.createGain();
        master.gain.setValueAtTime(.0001,now);master.gain.exponentialRampToValueAtTime(kind==='start'?.18:.13,now+.025);master.gain.exponentialRampToValueAtTime(.0001,now+(kind==='start'?1.05:.58));master.connect(context.destination);
        const notes=kind==='start'?[196,293.66,392,523.25]:[659.25,783.99,987.77];
        notes.forEach((frequency,index)=>{const oscillator=context.createOscillator(),gain=context.createGain(),start=now+index*(kind==='start'?.13:.09);oscillator.type=kind==='start'?'sawtooth':'sine';oscillator.frequency.setValueAtTime(frequency,start);gain.gain.setValueAtTime(.0001,start);gain.gain.exponentialRampToValueAtTime(kind==='start'?.34:.42,start+.018);gain.gain.exponentialRampToValueAtTime(.0001,start+(kind==='start'?.42:.25));oscillator.connect(gain);gain.connect(master);oscillator.start(start);oscillator.stop(start+(kind==='start'?.46:.3))});
...
export default function Game() {
    ...
    const matchStartedAt = useRef(Date.now()); const startSoundMatchId=useRef<string|null>(null);const previousTurn=useRef<number|null>(null); const [ping, setPing] = useState<number | null>(null); const [reportTarget, setReportTarget] = useState<number | null>(null); const [reportReason, setReportReason] = useState('AFK'); const [reportMessage, setReportMessage] = useState(''); const [state, setState] = useState<GameState>(() => { const roomId = searchParams.get('mode') === 'room' ? searchParams.get('room') : null; if (roomId) { try { const saved = localStorage.getItem(`room-game:${roomId}`); if (saved) return JSON.parse(saved) as GameState } catch { } } const total = Math.min(8, Math.max(6, Number(searchParams.get('players') || 6))); return newGame(profile?.displayName || 'Bạn', total - 1, GOLD_MINE_MAP) }); const [selected, setSelected] = useState<number | null>(null); const [isFullscreen, setIsFullscreen] = useState(false); const [aiThinking, setAiThinking] = useState(false); const [showFullscreenPrompt, setShowFullscreenPrompt] = useState(() => window.matchMedia('(max-width: 760px)').matches && localStorage.getItem('goldmine-fullscreen-choice') !== 'normal'); const [rememberFullscreen, setRememberFullscreen] = useState(false); const [showPlayersModal, setShowPlayersModal] = useState(false); const [showLeaveConfirm, setShowLeaveConfirm] = useState(false); const [special, setSpecial] = useState<SpecialKind | null>(null); const [targetPlayer, setTargetPlayer] = useState<number | null>(null); const [mySwapCard, setMySwapCard] = useState<number | null>(null); const [sabotageMode, setSabotageMode] = useState(false); const [statsSaved, setStatsSaved] = useState(false); const [matchRewards, setMatchRewards] = useState<Record<string, MatchReward> | null>(null);
    ...
    useEffect(()=>{if(roomAccess!=='allowed'||state.winner||startSoundMatchId.current===state.matchId)return;startSoundMatchId.current=state.matchId;previousTurn.current=state.turn;void playGameCue('start')},[roomAccess,state.matchId,state.winner]);
    useEffect(()=>{if(roomAccess!=='allowed'||state.winner)return;const before=previousTurn.current;previousTurn.current=state.turn;if(before!==null&&before!==state.turn&&state.turn===humanIndex)void playGameCue('turn')},[state.turn,humanIndex,state.winner,roomAccess]);
