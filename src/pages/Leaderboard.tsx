...
export default function Leaderboard(){
 ...
 return <section className="ranking-page"><div className="section-heading"><span>XẾP HẠNG TOÀN MÁY CHỦ</span><h1>BẢNG XẾP HẠNG RANK</h1><p>Xếp hạng theo Rank Point; level và EXP là tiến trình tích lũy riêng.</p></div>{loading?<div className="panel empty-state">Đang tải bảng xếp hạng...</div>:<><div className="podium">{data.slice(0,3).map((x,i)=>{const Icon=i===0?Crown:i===1?Medal:Trophy;return <Link to={`/profile/${x.uid}`} className={`pod p${i+1}`} key={x.uid}><Icon/><PlayerIdentity player={{name:x.displayName,photoURL:x.photoURL,rank:x.rank,equipped:x.equipped}}/><span>{x.rankPoints} RP</span></Link>})}</div><div className="ranking-table"><div className="rank-head"><span>HẠNG</span><span>NGƯỜI CHƠI</span><span>RANK</span><span>LEVEL</span><span>THẮNG</span></div>{data.map((x,i)=><Link to={`/profile/${x.uid}`} className="rank-row" key={x.uid}><b>#{i+1}</b><PlayerIdentity compact player={{name:x.displayName,photoURL:x.photoURL,rank:x.rank,equipped:x.equipped}}/><span>{x.rankPoints} RP</span><span>Level {levelFromExp(x.exp)}</span><span>{x.wins}/{x.gamesPlayed}</span></Link>)}</div></>}</section>;
}
