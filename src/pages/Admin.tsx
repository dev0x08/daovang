 ...
 return <section className="admin-page app-page">
  ...
  <section className="panel app-section admin-workspace">
   ...
   <div className={`admin-table-body ${activeTab}`}>
    ...
    {activeTab==='guilds'&&visibleGuilds.map(guild=><article key={guild.id}><span><b>{guild.tag&&`[${guild.tag}] `}{guild.name}</b><small>{guild.description||'Chưa có giới thiệu'}</small></span><span>{guild.ownerName||guild.ownerId}</span><strong>{guild.memberCount}</strong><div><button className="admin-icon-button" title="Chỉnh sửa Guild" onClick={()=>setGuildDraft({id:guild.id,name:guild.name,tag:guild.tag,description:guild.description,ownerId:guild.ownerId})}><Edit3/></button><button className="btn btn-danger btn-small" onClick={()=>ask({title:'XÓA GUILD',message:`Giải tán “${guild.name}” và xóa toàn bộ dữ liệu?`,confirmLabel:'XÓA GUILD',action:()=>disbandGuild(guild.id)})}><Trash2/> XÓA</button></div></article>)}
