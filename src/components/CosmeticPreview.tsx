import { Anchor, Crown, Ghost, Shield, TreePine, UserRound } from 'lucide-react';
import { cosmeticClass, ShopItem } from '../lib/shop';
import GuildBadgeMark from './GuildBadgeMark';
...
export default function CosmeticPreview({item,name='Anh Nguyễn',photoURL='',className=''}:{item:ShopItem;name?:string;photoURL?:string;className?:string}){
 ...
 if(item.category==='nameplate'){const PlateIcon=item.id==='plate-ocean'?Anchor:item.id==='plate-royal'?Crown:item.id==='plate-forest'?TreePine:item.id==='plate-phantom'?Ghost:Shield;return <div className={`cosmetic-demo cosmetic-demo-plate ${cosmeticClass(item.id)} ${className}`}>{avatar}<span><b>{name}</b><small>THỢ MỎ · LEVEL 1</small></span><i className="nameplate-scene"><PlateIcon aria-hidden="true"/></i></div>}
 ...
 if(item.category==='badge')return <div className={`cosmetic-demo cosmetic-demo-guild-badge ${className}`}><GuildBadgeMark id={item.id}/><span><b>{name}</b><small>GUILD BADGE</small></span></div>;
